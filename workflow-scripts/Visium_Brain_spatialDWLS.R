# Visium_Brain_deconvolution
# set path ------------------------------------------
rm(list=ls());options(stringsAsFactors=FALSE)
project_dir <- rprojroot::find_rstudio_root_file()
raw_dir <- file.path(project_dir,"datasets")
...
#############################test visium
#unzip(file.path(raw_dir,"mouse_brain_visium/raw_feature_bc_matrix.zip"))
raw_matrix <- get10Xmatrix("raw_feature_bc_matrix",gene_column_index = 2)
spatial_results <- fread(file=file.path(raw_dir,"mouse_brain_visium","tissue_positions_list.csv"))
python_path <- "/home/zhangfeng/bin/miniconda3/envs/R/bin/"
...
load(file.path(raw_dir,"mouse_brain_visium","sig_ct_exp.RData"))
